...
function goonj_handle_user_identification_form() {
    ...
    try {
		...
		if (!empty($recentCamp)) {
			...
			$redirect_url = get_home_url() . "/collection-camp-intent-details/#?Collection_Camp_Intent_Details.Created_By=" . $foundContacts['id'] . '&message=collection-camp-page&Collection_Camp_Intent_Details.Name=' . $display_name . '&Collection_Camp_Intent_Details.Contact_Number='. $phone;
		}
		wp_redirect($redirect_url);
		exit;
	} catch (Exception $e) {
