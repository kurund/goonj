document.addEventListener("DOMContentLoaded", function () {
  function checkSelect2ForOther() {
    ...
    if (select2Choices) {
      const selectedItems = select2Choices.querySelectorAll(
  ...
  function toggleAfFieldVisibility(shouldShow) {
    const afField = Array.from(document.querySelectorAll("af-field")).find(
      (field) => field.getAttribute("name") === "Volunteer_fields.Others_Skills"
    );
